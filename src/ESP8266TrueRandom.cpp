...
#define randomBitRaw() ((int)RANDOM_REG32 & 1)
/*
FORCE_INLINE ICACHE_FLASH_ATTR int ESP8266TrueRandomClass::randomBitRaw(void) {
  // Needed to keep wifi stack running smoothly
  // And to avoid wdt reset
  
  if (lastYield == 0 || millis() - lastYield >= 50) { //50
    yield();
    lastYield = millis();
  }
