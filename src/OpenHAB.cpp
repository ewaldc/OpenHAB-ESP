#include "OpenHAB.h"
// Following variables are generated by ESP OpenHab GenConfig in main.h and included in main.cpp
extern const uint8_t itemCount, groupCount, groupItemCount, pageCount;
...
extern uint8_t groupItems[];
extern const char* pages[];
extern JsonObject pageObj[];
extern void handleStateChange(uint8_t itemIdx, const char *prevState);
...
const char *OpenHab::functionOR(Group &group, uint8_t itemIdx, uint8_t *numItems) {
	int count = 0;
	...
	for (uint8_t n = 0; n < group.itemCount; n++) {
		uint8_t itemIdx = groupItems[n + group.items];
...
FORCE_INLINE float OpenHab::functionAVG(Group &group) {
	DbgPrint(F(" - functionAVG"));
	float avg = 0;
	uint8_t count = group.itemCount;
	for (uint8_t n = 0; n < count; n++)
		avg += strtof(itemStates[groupItems[n + group.items]], nullptr);
...
void OpenHab::setState(uint8_t itemIdx, const char *state, bool updateGroup) {
	...
	DbgPrint(F(" - new state: "), p);
	itemStates[itemIdx] = p;	// Update state
	//if (state) _callback_function(itemIdx, prevState);	// call back user code
	if (state) handleStateChange(itemIdx, prevState);	// call back user code
...
FORCE_INLINE void OpenHab::updateItem(const JsonVariant itemObj, const JsonVariant widgetObj, const char*pageId) {
	...
	DbgPrint(F(" - itemIdx: "), itemIdx); DbgPrint(F(" - type: "), item.type);	DbgPrint(F(" - state: "), item.state); 
	DbgPrint(F(" - label: "), labelObj.as<const char *>()); DbgPrint(F(" - pageId: "), pageId); DbgPrint(F(" - refCount: "), item.refCount);
	...
	*/	
	...
	if (ssid) { // We have an SSID, connect to this AP
		if (!WiFi.enableSTA(true)) { //enable STA failed
			Serial.println(F("ERROR: station mode enablement failed"));
		} else { 
			wifi_station_set_hostname(APssid);
			WiFi.begin(ssid, pwdDecoded);
			//while (WiFi.status() != WL_CONNECTED) delay(500);
			if (WiFi.waitForConnectResult() == WL_CONNECTED) {
				Serial.printf_P(PSTR("Connected to AP:%s (MAC:%s, name:%s)\n"),
					ssid,  WiFi.macAddress().c_str(), WiFi.hostname().c_str());
	...
	Serial.println(F("Falling back to soft AP"));
  	WiFi.mode(WIFI_AP);
  	WiFi.enableSTA(false);
	WiFi.disconnect(true);
	if (APssid) { // We  stat
		if (!WiFi.enableAP(true)) { //enabling of Soft AP failed
			Serial.println(F("ERROR: unable to create AP"));
			return false;
		}
		...
		Serial.println(F("ERROR: can not configure Soft AP"));
		WiFi.softAP(APssid, pwdDecoded, 13, false, 4);
    	MDNS.begin(APssid);
		wifi_station_set_hostname(APssid);
    	WiFi.hostname(APssid);
		Serial.printf_P(PSTR("Soft AP name:%s\n"), WiFi.hostname().c_str());
...
out:
	delete pwdDecoded;
	Serial.printf_P(PSTR("Network initialization complete, listening to %s:%d\n"), _espIP.toString().c_str(), _port);
	delay(100);
	//wifi_set_event_handler_cb(wifiEventHandler);

	SPIFFS.begin();
	Serial.println(F("Loading sitemap"));
	GetSitemapsFromFS();

	Serial.println(F("Initializing states and labels"));	// Second pass set state
...
void OpenHab::SSEdisconnect(WiFiClient client, Subscription &subscription) {
	DbgPrint(F(" - client no longer connected, remove subscription for page: "));
	isValidIndex(subscription.pageIdx) ? DbgPrintln(pages[subscription.pageIdx]) : DbgPrintln(subscription.pageIdx);
	if (client) client.stop();
	DbgPrint(F(" - after client stop"));
	subscription.clientIP = 0;
	if (subscription.uuidStr) free(subscription.uuidStr);
	DbgPrint(F(" - after uuid free"));
...
void OpenHab::SSEBroadcastItemChange(const char *state, const char *prevState, uint8_t itemIdx) {
	...
	for (uint8_t channel = 0; channel < SSE_MAX_CHANNELS; channel++) {
    	...
    	if (subscription.client && subscription.client.connected()) {	// Only broadcast if client still connected
			uint8_t pageIdx = subscription.pageIdx;
			if (isValidIndex(pageIdx)) {	// Basic UI
				uint8_t refCount = item.refCount;
				for (uint8_t n = 0; n <= refCount; n++)
					if (!strcmp_P(pages[pageIdx], itemref[n].pageId)) { // finds a matching pageId
						String itemStr;
						Serial.printf_P(PSTR("Broadcast Basic UI page change to client IP %s on channel %d for pageId %s and state change: %s --> %s\n"),
							IPaddrstr.c_str(), channel, itemref[n].pageId, prevState, state);
						serializeJson(itemref[n].obj, itemStr);
						DbgPrint(F(" - serialize OK"));
						int offset = itemStr.indexOf(F("\"state\":"));
						DbgPrint(F(" - itemstr with offset: "), offset);
						DbgPrintln(F(" - page: "), pages[pageIdx]);
						subscription.client.printf_P(PSTR("event: event\ndata: {\"widgetId\":\"%s\",\"label\":\"%s\",\"visibility\":true,\"item\":{%s,\"sitemapName\":\"%s\",\"pageId\":\"%s\"}\n\n"),
							...
							itemStr.substring(offset).c_str(), pages[0], pages[pageIdx]);
						/*
				...
				Serial.printf_P(PSTR("Broadcast HabPanel UI page change to client IP %s and state change: %s --> %s\n"),
					IPaddrstr.c_str(), state, prevState);
...
void OpenHab::handleSSEAll(const char *uri, bool isHabPanel) {
	...
	uint8_t channel;
	for (channel = 0; channel < SSE_MAX_CHANNELS; channel++)
		if (isHabPanel && !_subscription[channel].clientIP) break;
	...
	if (channel == SSE_MAX_CHANNELS) {	// IP address and UUID did not match, reject this client
   		Serial.println(F("\nERROR: No free slots or unregistered client attempts to listen"));
	   	return handleNotFound();
 	}
...
void OpenHab::handleChart(const char *uri) {
	DbgPrintRequest(F("handleChart"));
	if (strlen(uri) == 6) return SendFile(F("/chart.svg"), IMAGE_SVG);
	SendFile(uri, AUTO);
}
...
void OpenHab::handleAll() {
	...
	if (strncmp_P(uri, PSTR("/chart"), 6) == 0)	return handleChart(uri);
	//if (strcmp_P(uri, PSTR("/chart")) == 0)	return SendFile(F("/chart.svg"), IMAGE_SVG);
